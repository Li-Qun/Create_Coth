...
- (void) getTopList
{
    ...
    [request setCompletionBlock:^{
        ...
        [self getList:tableview  index:nil page:1];
...
- (void)buttonSlected:(UIButton *)sender
{
    ...
    if (index == 0) {
        ...
        [self getList:tableview  index:[NSString stringWithFormat:@"%d",index ] page:1];
    }
...
- (void)createClearView
{
    ...
    submit.titleLabel.font = [UIFont systemFontOfSize:9];
...
- (void)createScrollTab
{
    ...
    for (int i=0; i<titielArray.count; i++) {
        ...
        titleButton.titleLabel.font = [UIFont fontWithName:@"Helvetica-Bold" size:10];
...
{
    myScrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, 26, 320, self.view.frame.size.height-85)];
